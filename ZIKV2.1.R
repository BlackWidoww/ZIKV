###############################################################################
######                      Zika Virus Model 2.1                         ######
######              Working Copy to Play With Bite Rates                 ######
######                                                                   ######
###############################################################################


### Assuming that transmission rate beta_V*Host 
###     = (probability of vector biting host)*constant*(bites per day) 
### such that a constant mosquito biting rate is spread between a preferred host and 
...
pCH = .8; pIH = .7              # probability of vector i biting human host
rCH = pCH*rC; rCD = (1-pCH)*rC  # biting rate of competent vector on each host
rIH = pIH*rI; rID = (1-pIH)*rI  # biting rate of incompetent vector on each host 

tC = .1; tI = .01           # transmission probability per vector


betaHH = 0.5                # human-human transmission rate (sexual transmission)
betaCH = tC*rCH; betaIH = tI*rIH
betaCD = tC*rCD; betaID = tI*rID
betaHC = 0.5; betaHI = 0.05
